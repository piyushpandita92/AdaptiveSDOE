"""
Driver for different QoIs where:
idx: 1 is the expectation of the function,
idx: 2 is the expectation of the function,
idx: 3 is the maximum of the function.
"""
import matplotlib
matplotlib.use('agg')
import numpy as np
...
if __name__ == '__main__':
	...
	a = np.array([0])
	b = np.array([1.])
	X_init = lhs(dim , n, criterion='center') 
	...
	sigma = 0.2
	num_it = 15
	out_dir = get_out_dir_name(n=n, num_it=num_it, test_id=1, qoi=qoi_idx)
	if os.path.isdir(out_dir):
		shutil.rmtree(out_dir)
	os.makedirs(out_dir)
	bounds = {"b":b, "a":a}
	hmc_priors = {"ell_kern_variance_prior_list":[gpflow.priors.Gamma(1., 1.)],
	"ell_kern_lengthscale_prior_list":[gpflow.priors.Gamma(1., 1.)],
	"ss_kern_variance_prior_list":[gpflow.priors.Gamma(1., 1.)],
	"ss_kern_lengthscale_prior_list":[gpflow.priors.Gamma(1., 1.)], 
	"mean_func_ell_prior_list":[gpflow.priors.Gaussian(-2., 0.1)],
	"mean_func_ss_const_list":[0]}
	x_hyp = np.array([[.6]])
	kls = KLSampler(X_init,
                    ...
                    bounds=bounds,
					**hmc_priors)
	X, Y, kld, X_design, mu_qoi, sigma_qoi, models, samples = kls.optimize(num_designs=1000,
		verbose=1,
		plots=2,
		num_designs_ego=30,
		num_samp=50,
	...
	plt.xticks(np.arange(0, len(kld_max), step=5), np.arange(0, len(kld_max), step=5), fontsize=16)
	...
	for i in xrange(num_samp):
		y_samp[i, ], val, funcs = kls.obj_est(X_samp)
		y_samp_hyp[i, ], val, funcs, y_hyp = kls.obj_est_hyp(X_samp, x_hyp)
