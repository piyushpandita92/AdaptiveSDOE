...
if __name__=='__main__':
	...
	objective_true = Ex3Func(sigma=sigma_true)
	...
	gpflow.priors.Gamma(1., 1.)], 
	"mean_func_ell_const_list":[0., 0., 0.],
	"mean_func_ss_const_list":[0., 0., 0.]}
	x_hyp = .6 * np.ones(dim)[None, :]
	kls = KLSampler(X_init,
                    ...
                    jitter=1e-4,
                    kld_tol=1e-6,
                    func_name=os.path.join(out_dir, 'ex3'),
                    energy=0.95,
	...
	plt.scatter(true_mean, 0, c=sns.color_palette()[2], label='true value of the QoI')
	# plt.xticks([mu_qoi[-1]])
	plt.legend(fontsize=12)
