...
@NameScoped("conditional_full")
def conditional_full(Xnew, X, kern, f, full_cov=False, q_sqrt=None, whiten=False):
    """
...
@NameScoped("conditional_cov_full")
def conditional_cov_full(X1, X2, X, mu_ell_X1, mu_ell_X2, mu_ell_X, mu_ss_X1, mu_ss_X2, mu_ss_X, nonstat, V):
    ...
    for i in xrange(num_feat):
        ...
        ss_X_i = tf.exp(mu_ss_X[i])
        ss_X1_i = tf.exp(mu_ss_X1[i])
        ss_X2_i = tf.exp(mu_ss_X2[i])
        K_X_X = tf.multiply(K_X_X, nonstat.K(X_i, ell_X_i, ss_X_i, X_i, ell_X_i, ss_X_i))
        K_X_X1 = tf.multiply(K_X_X1, nonstat.K(X_i, ell_X_i, ss_X_i, X1_i, ell_X1_i, ss_X1_i))
        K_X_X2 = tf.multiply(K_X_X2, nonstat.K(X_i, ell_X_i, ss_X_i, X2_i, ell_X2_i, ss_X2_i))
        K_X1_X2 = tf.multiply(K_X1_X2, nonstat.K(X1_i, ell_X1_i, ss_X1_i, X2_i, ell_X2_i, ss_X2_i))
...
@NameScoped("nonstat_conditional_multidim_full")
def nonstat_conditional_multdim_full(Xnew, X, mu_ell_Xnew, mu_ell_X, mu_ss_Xnew, mu_ss_X, nonstat, V, full_cov=True):
    """
    ...
    for i in xrange(num_feat):
        ...
        ss_X_i = tf.exp(mu_ss_X[i])
        ss_Xnew_i = tf.exp(mu_ss_Xnew[i])
        K_X_X = tf.multiply(K_X_X, nonstat.K(X_i, ell_X_i, ss_X_i, X_i, ell_X_i, ss_X_i))
        K_X_X_new = tf.multiply(K_X_X_new, nonstat.K(X_i, ell_X_i, ss_X_i, Xnew_i, ell_Xnew_i, ss_Xnew_i))
        K_X_new_X_new = tf.multiply(K_X_new_X_new, nonstat.K(Xnew_i, ell_Xnew_i, ss_Xnew_i, Xnew_i, ell_Xnew_i, ss_Xnew_i))
    K_X_X = K_X_X + tf.eye(num_data, dtype=float_type) * settings.numerics.jitter_level
    NonStat_Lm = tf.cholesky(K_X_X)
    # Compute the projection matrix A
    ...
    # construct the conditional mean
    NonStat_fmean = tf.matmul(NonStat_A, V, transpose_a=True)
