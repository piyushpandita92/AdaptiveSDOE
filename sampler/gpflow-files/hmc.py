...
def sample_HMC(f, num_samples, Lmin, Lmax, epsilon, x0, verbose=False,
               ...
               return_logprobs=False, return_acc_ratio=False):
    """
    ...
    accept_count_track = np.ones(int((num_samples * thin) / 100))
    ...
    for t in range(1, num_samples * thin):
        ...
        if ((t+1) % 100) == 0:
            ...
            accept_count_track[int((t + 1) / 100 - 1)] = accept_count_batch / 100.
            accept_count_batch = 0
    ...
    if return_logprobs:
        if return_acc_ratio:
            return samples, logprob_track, accept_count_track
        else:
            return samples, logprob_track
    else:
        if return_acc_ratio:
            return samples, accept_count_track
        else:
            return samples